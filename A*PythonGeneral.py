...
class Puzzle:
    ...
    def accept(self): ##Allows general input and makes sure correct number of inputs and then puts into 2x2 matrix
        currentPuzzle = []
        for i in range(0,3): ## 3 lines of input
            temp = input().split(" ")
            currentPuzzle.append(temp)
        ##Very basic validation, not comprehensive
        if(len(currentPuzzle[0]) != 3 or len(currentPuzzle[1]) != 3 or len(currentPuzzle[2]) != 3 ):
    ...
    def AStar(self):
        print("Would you like to use Manhattan(M) or Euclidean(E) distance") ##User selects heuristic
        disChoice = input("")
        print("Please enter the start state matrix \n") ##As this is a general solution the user has to enter the matrix
        start = self.accept()
        print("Please enter the goal state matrix \n")
        goal = self.accept()
